      ...
      >
        ...
        >
          <Box sx={{ display: "flex", gap: 2 }}>
            ...
            <Chip label="Returned: 0" color="info" variant="outlined" />
          </Box>
          <Box>
            <Button
              component={Link}
              to="/management/quotation/create"
              variant="contained"
              color="primary"
            >
              Create Quotation
          ...
          <Table>
            <TableHead>
              <TableRow sx={{ backgroundColor: "#039BE5" }}>
                <TableCell sx={{ color: "white" }}>Quotation Number</TableCell>
                <TableCell sx={{ color: "white" }}>Client</TableCell>
                <TableCell sx={{ color: "white" }}>Person In Charge</TableCell>
                <TableCell sx={{ color: "white" }}>Receiver Status</TableCell>
                <TableCell sx={{ color: "white" }}>Authorization</TableCell>
                <TableCell sx={{ color: "white" }}>Approval Status</TableCell>
              ...
              {data.map((row, idx) => (
                <TableRow key={idx}>
                  ...
                  <TableCell>
                    ...
                    {row.receiverNote && (
                      <Box mt={1}>{getStatusChip(row.receiverNote)}</Box>
                    )}
                  </TableCell>
                  <TableCell>
                    {row.authorization?.map((auth, i) =>
                      auth ? (
                        <CheckCircle key={i} color="success" />
                      ) : (
