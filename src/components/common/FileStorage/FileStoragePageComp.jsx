...
export const FileStoragePageComp = (props) => {
    ...
    const { loading: reNameLoading, callUpdateFileName } =
        useCallUpdateFileName({
            fileId: fileId[0],
    ...
    const userExceedPlan =
        platform?.id && data?.users
            ? platform?.total_member > data?.users
            : false;
    ...
    return (
        <FileStorageWrapper className="fsw">
            ...
            {(!platform?.id || isAdmin) && (
                <Link href={`/${storageLink}settings?update=storage`}>
                    <Typography
                        variant="body2"
                        color="textSecondary"
                        gutterBottom
